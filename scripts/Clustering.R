...
# Tidies the data with dummy rows and row Ids

test_cluster <- test_data[,-1] # makes row names
test_cluster2 <- test_data[,-1]

test_cluster$MGI_ID <- NULL # deletes extra names
test_cluster$HGNC_ID <- NULL # deletes extra names
test_cluster$adHEMI_top_level_mp_name <- NULL # removes extra column
test_cluster$adHOM_top_level_mp_name <- NULL # removes extra column

test_cluster <- dummy_cols(test_cluster, 
                           select_columns = c('IMPC_adult_viability',
                                              'IMPC_embryo_viability',
                                              'Avana_mean_045',
                                              'IMPC_Viability',
                                              'Viable_Phenotypes_updated',
                                              'FUSIL_outliers',
                                              'FUSIL'),
...
rownames(test_cluster2) <- test_data[,1]  # assigns row names

# Plots a cluster of the test dataset
d <- dist(test_cluster, method = "euclidean") # distance matrix
...
# Human_rpkm clustering

# Loads data
human_rpkm <- read.csv("~/Uni/Level 7/Research Project/fusil-ml-main/Clustering/Human_rpkm.txt", row.names=1, sep="")

# clusters
d3 <- dist(human_rpkm, method = "euclidean") # distance matrix
fit4 <- hclust(d3, method="ward.D") 
plot(fit4)
dgroups3 <- cutree(fit4, k=5) # cut tree into 5 clusters
rect.hclust(fit4, k=5, border= 2:6)

# makes df of assigned groups
rpkm_groups <- data.frame(dgroups3)

write.csv(dgroups3,
          "C:\\Users\\yido6\\Documents\\Uni\\Level 7\\Research Project\\fusil-ml-main\\Clustering\\rpkm_groups.csv", 
          row.names = TRUE)
