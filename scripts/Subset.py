...
anno = pd.read_csv(r'Clustering\output data\allannotations_no_nan.csv')
newfusil = pd.read_csv(r'Classification\tmp\fusil.csv', index_col=0)
...
df5 = df4.drop(columns='Group')

df6 = pd.merge(left=anno,
               right=newfusil,
               how='left',
               left_on='hgnc_id',
               right_on='hgnc_id')

df6.drop_duplicates(subset="hgnc_id",
                    keep=False,
                    inplace=True)

# NOTE: Subsets the training data and drops '-' fusil groups
training = df6.dropna(subset=['fusil'])
training = training[training.fusil != '-']

training.to_csv(r'Classification\input data\trainingdata.csv', index=False)


# NOTE: Subsets the training data
test = df6[df6['fusil'].isnull()]

test.to_csv(r'Classification\input data\testdata.csv', index=False)
